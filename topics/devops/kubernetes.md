...
Building in multiple stages produces a smaller final image, e.g., compile in Golang builder image, copy binary into scratch/alpine. This reduces image size and keeps runtime clean.

### 6. How to persist data in Docker?

Use **volumes** or **bind mounts**; the container filesystem is ephemeral.
...
The smallest deployable unit in Kubernetes. A pod wraps one or more containers that share network namespace and volumes.
...
- If something fails, `kubectl rollout undo` reverts to the previous ReplicaSet.
...
API object that manages external access (usually HTTP/HTTPS) to services. Works with an Ingress Controller (nginx, traefik, etc.).
